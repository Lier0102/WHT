...
client.on('message', msg => {
  ...
  if(!msg.content.startsWith("Covid ")) return;

  let command = msg.content.split("Covid ");
  command = command[1].split(" ");

  if(msg.content == "Covid") {
    msg.reply("사용법 : Covid [명령어] \n더 많은 설명을 찾고싶다면 Covid help 또는 Covid 도움말 을 쳐주세요!")
  }

  if(msg.content == "Covid 도움말" || msg.content == "Covid help") {
    let HelpEmbed = new Discord.MessageEmbed()
      .setColor('#00ff9d')
      .setTitle('코로나봇 사용법')
      .setAuthor('Created By KISHA')
      .addFields(
        { name: 'Covid 도움말', value: '코로나봇 도움말을 가져옵니다.'},
        { name: 'Covid 코로나', value: '국내 총 코로나19 현황을 가져옵니다.'},
        { name: 'Covid 지역코로나', value: '지역별 총 코로나19 현황을 가져옵니다.'},
        { name: 'Covid 지역코로나 [지역이름]', value: '인자로 전달한 지역이름의 코로나19 현황을 가져옵니다.\n지역이름은 광역시(서울, 인천, 제주 등...)를 제외하고 도 단위입니다.'}
      )
